...
export default function TerminalPage() {
  ...
  const clearTerminal = (id: string) => {
    setTerminals(terminals.map(t => (t.id === id ? { ...t, history: [] } : t)));
  };
  ...
  const runCommand = () => {
    ...
    if (commandTrimmed === 'clear' || commandTrimmed === 'cls') {
      ...
      setTerminals(terminals.map(t => (t.id === activeTab ? { ...t, isRunning: true } : t)));
    ...
    if (updatedTerminal) {
      ...
      setTerminals(terminals.map(t => (t.id === activeTab ? { ...t, history: newHistory } : t)));
    }
  ...
  const changeTerminalType = (id: string, type: string) => {
    setTerminals(terminals.map(t => (t.id === id ? { ...t, type } : t)));
  };

  // Terminal adını güncelle
  const updateTerminalName = (id: string, name: string) => {
    setTerminals(terminals.map(t => (t.id === id ? { ...t, name } : t)));
  };

  // Aktif terminali durdur
  const stopTerminal = (id: string) => {
    setTerminals(terminals.map(t => (t.id === id ? { ...t, isRunning: false } : t)));
  };
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Komutları çalıştırın ve çıktılarını görüntüleyin</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => window.open('/dashboard/terminal/settings', '_blank')}
          >
      ...
      <Card className="border overflow-hidden">
        ...
        <CardContent className="p-0">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            {terminals.map(terminal => (
              <TabsContent key={terminal.id} value={terminal.id} className="m-0 outline-none">
