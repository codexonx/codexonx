...
export function AdminNav({ isCollapsed = false }) {
      ...
      return (
        <div key={item.label} className="space-y-1">
          ...
          >
            ...
            {!isCollapsed &&
              (openGroups[item.label.toLowerCase()] ? (
  ...
  return (
    <div
      className={cn('flex flex-col h-full border-r bg-background', isCollapsed ? 'w-16' : 'w-64')}
    >
      {/* Logo */}
      <div
        className={cn('flex h-16 items-center px-4 border-b', isCollapsed && 'justify-center px-2')}
      >
      ...
      <div className="flex-1 overflow-auto py-4 px-2">
        <nav className="space-y-2">{navItems.map((item, i) => renderNavItem(item, i))}</nav>
      </div>
