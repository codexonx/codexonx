  ...
  return new Promise(resolve => {
    ...
    setTimeout(() => {
      ...
      if (prompt.toLowerCase().includes('merhaba') || prompt.toLowerCase().includes('selam')) {
        responseText =
          'Merhaba! Size nasıl yardımcı olabilirim? Kod yazma, hata ayıklama veya geliştirme konularında yardım edebilirim.';
      } else if (
        prompt.toLowerCase().includes('javascript') ||
        prompt.toLowerCase().includes('js')
      ) {
...
Bu değişiklikleri yapmak projenizin bakımını kolaylaştıracaktır.`;
      ...
      inputLower.includes('geliştir') ||
      inputLower.includes('optimiz')
    ) {
      if (fileExt === 'js' || fileExt === 'jsx') {
...
Bu örnek, modern React uygulamalarında kullanılan hooks, veri yönetimi, optimizasyon ve component yapısını göstermektedir.`;
      ...
      responseText =
        'Sorunuzu aldım. Size daha iyi yardımcı olabilmem için projenizdeki spesifik bir problemden veya geliştirmek istediğiniz bir konudan bahsedebilir misiniz?';
    }
  }
